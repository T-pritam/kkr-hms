...
| **Status** | Rounds 1 – 6 answered (2026-09-22 / 25); rounds 1 – 7 answered (2026-09-22 / 25); **nothing open** ([`OPEN-QUESTIONS.md`](OPEN-QUESTIONS.md)). **Every CR is built and live** — CR-01 … CR-19, less CR-17 (dropped, Q-74). |
...
**Nothing is open** as of 2026-09-25: rounds 1 – 7 are all answered. New questions go in [`docs/OPEN-QUESTIONS.md`](OPEN-QUESTIONS.md).
...
|---|---|---|---|
| 1, 3 | Medicine: *"don't ask anyone… it is an expense… don't add not decided… drop the Lab & medicine container… alert: added as Expense only"* | **Medicine is always an expense.** No included/paid-directly question, no "not decided"; the Overview shows a Medicine expense line; saving says "Added as an expense only." Supersedes Q-82/Q-83/Q-86/Q-87 | `APP-FLOW-PRD.md` §1.3, §4.5 |
| 2 | Lab: *"in-house lab… income… add directly to payments with Lab group… a notes field (not mandatory)… it will add to the ledger as an individual row"*. Asked where it starts → **the Payments tab**; and, on review, *"lab and registration work the same way… keep both in the charges also, for a dummy bill"* | **Lab works exactly like the registration fee:** *Add lab test* on Payments writes a Charges line (reference), a payment (Lab) and its own ledger row (type Lab) together; the charge line is read-only and follows the payment | §4.7, §4.10, `lib/billing/linked-charge.ts` |
| 4 | Overview total *"x + y + z = A"* (payments, registration fee, lab) | Shown on the Overview; the same split feeds Finances money in, where Registration and Lab are now their own lines | §4.3, §4.14 |
| 5, 6 | Registration fee hard-coded at ₹100 while the catalogue says ₹300; patient 6/26 showed "collect" in Payments but a line in Charges | Asked → **nothing is written until the fee is collected; until then it is the catalogue's current price; collecting writes charge + payment + ledger together**, and the amount is then fixed. (The ₹100 came from the catalogue: its price changed 48 seconds after 6/26 was registered.) | §4.2 |
| 7 | *"Remove Add payment from the Ledger"* | Removed; payments are taken on the patient | §4.10 |
| 8 | Reception and the ledger. Asked → **reception keeps the Ledger, without the 5 totals** | Not shown, and not sent to reception's browser | §4.10 |
| 9 | The logo from the PDF letterhead. Asked where → **sidebar, browser tab, login page** | Cropped from the letterhead emblem | — |
| 10 | Patient and doctor lists cut off on a square screen, no scroll | The table's actions ran off the right with the sidebar open, and its sideways scrollbar sat under the last row. Below 1280 px the compact list is used; the same fix on 5 more lists; the Ledger and Petty cash pin their Actions column | BUGS #78 |
| — | The two lab charges already in Charges (₹800, ₹400) | **Delete them** (client's answer) | migration B |
| — | Finances: "Lab & Medicine" | Becomes **Medicine**; Lab is income, its own line | §4.14 |

### 9.0b Round 7 — answered 2026-09-25

| Q | Asked | Answer | Recorded in |
|---|---|---|---|
| Q-99 | What should the **Dashboard** be? It was a placeholder: four tiles that always read 0 (BUGS #73). A remove it · B build a small admin dashboard · C leave it | *"Go with what is best."* → **A: removed.** Everyone, the admin included, lands on **Patients** after signing in; the sidebar entry is gone and `/dashboard` redirects to Patients for old bookmarks | `APP-FLOW-PRD.md` §1.4, §2 |
...
| 2026-09-25 | **Pre-release fixes** on `feature/v2-release-fixes`. Security: a refresh token no longer opens a request (#1); a password change or admin reset ends every other session within ten minutes, and deactivating an account now ends its sessions (#3, `users.token_version`, migration `20260925000004`); validating a reset link can no longer change a password (#4); the admin user search, PATCH and page size are fixed (#5, #6, #70); three id filters are validated (#74); the retired `upload-case-sheet` edge function refuses everything (#69). Correctness: a patient with records cannot be deleted (#9); an edited visit cannot predate joining (#14); one bill per patient, with a unique index (#22, `20260925000005`); fee payment and merge checks (#27, #44, #45, plus refusing to merge a paid fee); the employee CSV import reads real CSV, skips duplicates and issues codes (#54). Polish: the advance log names every giver (#71); lab technician accounts (#72). Lint passes (0 errors). #68 (database access) is held for a decision, with a plan and workaround in `docs/SECURITY-DB-ACCESS.md`; Q-99 asked what the Dashboard should be. 62 files, 1,843 tests passing, none expected to fail | Claude |
