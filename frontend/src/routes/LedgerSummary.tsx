...
export default function DailyLedgerSummaryPage() {
  ...
  const fetchDailySummary = async () => {
    try {
      setLoading(true)
      const response = await apiFetch(`/api/ledger/daily-summary/${selectedDate}?user_id=${user?.id}`, {
        credentials: 'include'
      })
      const data = await response.json()
  ...
  const handleDeleteTransaction = async (id: string) => {
    ...
    try {
      const response = await apiFetch(`/api/ledger/transactions/${id}`, {
        method: 'DELETE',
        credentials: 'include'
      })
  ...
  const handleVerifyTransaction = async (id: string) => {
    try {
      const response = await apiFetch(`/api/ledger/transactions/${id}/status`, {
        method: 'PUT',
