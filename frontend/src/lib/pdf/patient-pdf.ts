...
function box(h: H, x: number, y: number, w: number, label: string, value: string, accent: [number,number,number] = C.blue): void {
  h.doc.setFillColor(248, 250, 253)
  h.doc.setDrawColor(...C.border)
  h.doc.setLineWidth(0.35)
  h.doc.rect(x, y, w, BOX_H, 'FD')
...
function boxRow(h: H, items: { label: string; value: string; accent?: [number,number,number] }[]): void {
  const n = items.length
  const w = (h.cw - GAP * (n - 1)) / n
  items.forEach((it, i) => box(h, M + i * (w + GAP), h.y, w, it.label, it.value, it.accent || C.blue))
  h.y += BOX_H + 8
}
...
const RE = M + 269  // right edge of content
