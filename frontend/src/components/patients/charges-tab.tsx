...
export default function ChargesTab({ patientId, billing, onCreateBilling }: ChargesTabProps) {
  ...
  const fetchCharges = async () => {
    try {
      const response = await apiFetch(`/api/patients/${patientId}/charges?billing_id=${billing.id}`);
      if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        const response = await apiFetch(`/api/patients/${patientId}/charges/${editingId}`, {
          method: 'PATCH',
        ...
        const response = await apiFetch(`/api/patients/${patientId}/charges`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            ...formData,
  ...
  const handleDelete = async (chargeId: string) => {
    ...
    try {
      const response = await apiFetch(`/api/patients/${patientId}/charges/${chargeId}`, {
        method: 'DELETE',
      });
