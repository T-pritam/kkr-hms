...
export default function BillingSettlementTab({
  ...
  const fetchSettlements = async () => {
    try {
      const response = await apiFetch(`/api/patients/${patientId}/settlements?billing_id=${billing.id}`);
      if (response.ok) {
        const data = await response.json();
        setSettlements(data);
      }
  ...
  const handleSyncDoctorVisits = async () => {
    setSyncing(true);
    try {
      const response = await apiFetch(`/api/patients/${patientId}/settlements/sync`, {
        method: 'POST',
  ...
  const handleConfirmSettlePricing = async () => {
    ...
    setLoading(true);
    try {
      const response = await apiFetch(`/api/doctor-settlements/${settleData.settlement_id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleUpdateSettlement = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiFetch(`/api/doctor-settlements/${editingSettlement.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleDeleteSettlement = async (settlementId: string) => {
    ...
    try {
      const response = await apiFetch(`/api/doctor-settlements/${settlementId}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
