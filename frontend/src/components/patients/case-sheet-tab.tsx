...
export default function CaseSheetTab({ patientId, billing, onStatusChange }: CaseSheetTabProps) {
  ...
  const uploadToR2 = async (file: File): Promise<{ url: string; filename: string } | null> => {
    ...
    try {
      ...
      const response = await apiFetch(`/api/patients/${patientId}/case-sheets/upload`, {
        method: 'POST',
        body: formData,
      });
    ...
    try {
      if (isEditing && caseSheet) {
        ...
        if (formData.case_sheet_file) {
          ...
          const response = await apiFetch(`/api/patients/${patientId}/case-sheets/${caseSheet.id}`, {
            method: 'PATCH',
            body: patchFormData,
          });
        ...
        if (response.ok) {
          ...
          const statusRes = await apiFetch(`/api/patients/${patientId}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
